...
% Difference between a version and b version is that for the b version
% we fix Str for all rep instead of changing every time.
% Difference between b version and c version is that in this version
% we add an extra loop to see how the averge optimal alpha and the std dev
% of the optimal alpha depends on a given parameter. The parameters that
% are considered are: rho, N, g, sigmaW. We start out with rho here

clear
clc

% COMPARATIVE STATICS PARAMETER
csiterTot=19;
maxparameter=0.9;
minparameter=-0.9;
parameter=zeros(csiterTot,1);
for i=1:csiterTot
    parameter(i,1)=minparameter+(maxparameter-minparameter)*(i-1)/(csiterTot-1);
end

% COMPARATIVE STATICS OUTPUT VECTORS

OptimalAlphasMeansVector=zeros(csiterTot,1);
OptimalAlphasStdDevsVector=zeros(csiterTot,1);
SigmaWMeanVector=zeros(csiterTot,1);
SigmaWStdDevVector=zeros(csiterTot,1);
GTdistMeanMean=zeros(1,csiterTot);
GTdistSDevMean=zeros(1,csiterTot);
GTdistMax=zeros(1,csiterTot);
GTdistMin=zeros(1,csiterTot);

for csiter=1:csiterTot
...
reptot=5; %Number of times this program runs (diff W each time)
...
beta= 2.5;  % New Benchmark = 1.8
gamma=0.04; % New Benchmark = 0.04
...
Abar=19; %Number of Alphas to consider. Each one for a diff scoring method
...
sbar=20; %Number of Monte Carlo simulations (fixed Str, W; diff WT)
...
rho = parameter(csiter,1);
cwght = 2 * rho ;  %Correlation Weight: 0=Uniform;1=twice the neighbor;2=4times 
...
GTdistMean = zeros(1,reptot); %Average of average distance between teams that play each other
GTdistSDev = zeros(1,reptot); %SDev of average distance between teams that play each other
...
% Compute linear and geometric strength of schedule
GPT=GT*ones(N,1);
LinSoS=(GT*Str)./GPT; % Linear Strength of Schedule vector
GeoSoS=ones(N,1); % Geometric Strength of Schedule vector
for i=1:N
    for j=1:N
        if GT(i,j)==1
            GeoSoS(i,1)=GeoSoS(i,1)*(Str(j,1)^(1/GPT(i)));
        elseif GT(i,j)==2
            GeoSoS(i,1)=GeoSoS(i,1)*((Str(j,1)^(1/GPT(i)))^2);
        end
    end
end
if rep==1
...
% figure
% scatter(log(Str),log(GeoSoS))
if rep==1
    figure
    plotregression(log(Str),log(GeoSoS))
end 
end    % End of Iteration over all 'rep' up to 'reptot'.
toc

GTdistMeanMean(1,csiter) = mean(GTdistMean(1,:));
GTdistSDevMean(1,csiter) = std(GTdistMean(1,:));
GTdistMax(1,csiter) = max(max(GTdistMeanMC));
GTdistMin(1,csiter) = min(min(GTdistMeanMC));
...
txt = ['  N = ' num2str(N) ',  g = ' num2str(mean(g)) ',  \rho = ' num2str(rho) ',  \sigma_{W} = ' num2str(mean(sigmaW)) ',   Sims = ' num2str(sbar)];
xloc = Alphas(1);
yloc = max(max(VTNDifSDevCSAvRepTot(2:reptot+1,:)));
text(xloc,yloc,txt)
title('Efficiency of \alpha')
xlabel('\alpha')
ylabel('Standard deviation from true win percentage')
...
OptimalAlphasMeansVector(csiter,1)=mean(OptimalAlphas);
OptimalAlphasStdDevsVector(csiter,1)=std(OptimalAlphas);
SigmaWMeanVector(csiter,1)=mean(sigmaW);
SigmaWStdDevVector(csiter,1)=std(sigmaW);
end

figure
plot(parameter,OptimalAlphasMeansVector)
txt = ['  N = ' num2str(N) ',  g = ' num2str(mean(g)) ',  \sigma_{W} = ' num2str(mean(sigmaW)) ',   Sims = ' num2str(sbar)];
xloc = parameter(1,1);
yloc = max(OptimalAlphasMeansVector)*0.95;
text(xloc,yloc,txt)
title('Most Efficient \alpha')
xlabel('\rho')
ylabel('Most Efficient \alpha')

[mean(g),N,min(GTdistMin),max(GTdistMax)]

figure
plot(parameter,GTdistMeanMean)
title('Average of the Average Distance between teams that play each other')
xlabel('\rho')
ylabel('Average of the average dist')
